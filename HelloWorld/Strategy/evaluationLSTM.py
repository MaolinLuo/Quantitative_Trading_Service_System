...
def prepareModel(ts_code, start_date="20190101", end_date="20220811", train_test_rate = 0.8, epoch=100, steps=3):
    ...
    lstm = lstm_model.LSTMModel(num_of_layers=num_of_layers)
    lstm.to(device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(lstm.parameters(), lr=lr)

    # 模型的训练与评估
    lstm = train(lstm, epoch, train_data, criterion, optimizer, device)
    torch.save(lstm, './lstm.pt')
    real_data, pred_data = evaluator(lstm, test_data, device, steps=steps)
