...
def prepareModel(ts_code, start_date="20190101", end_date="20220811", train_test_rate = 0.8, epoch=100, steps=3):
    ...
    rnn = rnn_model.RNNModel(num_of_layers=num_of_layers)
    rnn.to(device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(rnn.parameters(), lr=lr)

    # 模型的训练与评估
    rnn = train(rnn, epoch, train_data, criterion, optimizer, device)
    torch.save(rnn, './rnn.pt')
    real_data, pred_data = evaluator(rnn, test_data, device, steps=steps)
