           ...
           for data in self.datas:
            ...
            # 低于35，买入
                if self.Mfi[data._name][0]<35:
                    ...
                    print(data._name)
                    temp = util.return_trade_dict(data, "buy", stakesize)
...
def run_mfi(ts_code_list,startdate,enddate):
    ...
    for ts_code in ts_code_list:
        stock=util.getmsidata(ts_code)
        data = TusharePdData(dataname=stock, fromdate=util.datetime.date(start_year, start_month, start_day), todate=util.datetime.date(end_year, end_month, end_day))
